    ...
    try {
      ...
      const generatedSQL = await sqlGenerationTool.execute({
        context: {
          ...
          databaseSchema: {
            ...schema,
            relationships: Array.isArray(schema.relationships) ? schema.relationships : [],
            indexes: Array.isArray(schema.indexes) ? schema.indexes : [],
            rowCounts: Array.isArray(schema.rowCounts) ? schema.rowCounts : [],
          },
        },
        runtimeContext: runtimeContext || new RuntimeContext(),
      });
...
export const databaseQueryWorkflow = createWorkflow({
  ...
  steps: [getConnectionStep, introspectDatabaseStep, generateSQLStep, reviewAndExecuteStep],
});
